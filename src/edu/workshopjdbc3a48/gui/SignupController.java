...
String co;
    ...
    @FXML
    private PasswordField tfMdp_si;
    @FXML
    ...
    @FXML
    private PasswordField Confirm_mdp;
    @FXML
    private Label conf_mdp;
    @FXML
    private ImageView show_pass;
    @FXML
    private TextField mdp_si;
...
if (pass_vis ==0)
{mdp_si.setVisible(false);
tfMdp_si.setVisible(true);
}
else
{
mdp_si.setVisible(true);
tfMdp_si.setVisible(false);
}
    }    
    ...
    private void AjoutUser(ActionEvent event) throws IOException {
      ...
      if (tfNom_si.getText().matches(".*\\d+.*")==true||!Character.isUpperCase(tfNom_si.getText().charAt(0)))
      {
      check1=false;
      tfNom_si.setStyle("-fx-background-color: #ff0000;");
      if (!Character.isUpperCase(tfNom_si.getText().charAt(0)))
      ...
      if (tfPrenom_si.getText().matches(".*\\d+.*")==true||!Character.isUpperCase(tfPrenom_si.getText().charAt(0)))
      {
      check2=false;
      tfPrenom_si.setStyle("-fx-background-color: #ff0000;");
        if (!Character.isUpperCase(tfPrenom_si.getText().charAt(0)))
      ...
      if (!tfNum_si.getText().matches(".*\\d+.*")==true)
      {
      check3=false;
      tfNum_si.setStyle("-fx-background-color: #ff0000;");
      e_num.setText("Format num incorrect!");
      }
       else {
       check3=true;
      ...
      if (tfEmail_si.getText().matches("^[\\w-\\.]+@([\\w-]+\\.)+[\\w-]{2,4}$")==false)
      {
      check6=false;
      ...
      if (tfMdp_si.getText().equals(Confirm_mdp.getText()))
      {
       check_mdp=true;
        conf_mdp.setText("");
      tfMdp_si.setStyle("-fx-background-color: #ffffff;");
       Confirm_mdp.setStyle("-fx-background-color: #ffffff;");
      }
      else {
      check_mdp=false;
         Confirm_mdp.setStyle("-fx-background-color: #ff0000;");
         tfMdp_si.setStyle("-fx-background-color: #ff0000;");
         conf_mdp.setText("Veuillez confirmer votre mot de pass !");
...
try {
    ...
    private void Show(MouseEvent event) {
        ...
        mdp_si.setVisible(true);
tfMdp_si.setVisible(false);
mdp_si.setText(tfMdp_si.getText());
        }
        else if (pass_vis==1)
        {pass_vis=0;
        tfMdp_si.setText(mdp_si.getText());
        mdp_si.setVisible(false);
tfMdp_si.setVisible(true);}
    }
    }
